...
class GeofenceTransitionsJobIntentService : JobIntentService(), CoroutineScope {
    ...
    override fun onHandleWork(intent: Intent) {
        ...
        if (geofencingEvent?.hasError()!!) {
            Log.d("geofencingEvent", "error: ${geofencingEvent.errorCode}")
            return
        }
        if (geofencingEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
            Log.d("TAG", "onHandleWork: User enter area")
    ...
    private fun sendNotification(triggeringGeofences: List<Geofence>) {
        ...
        Log.d("TAG", "sendNotification: Display Notification: $requestId")
...
//        Interaction to the repository has to be through a coroutine scope
        CoroutineScope(coroutineContext).launch(SupervisorJob()) {
            ...
            if (result is Result.Success<ReminderDTO>) {
                ...
                Log.d("TAG", "sendNotification: Send  Notification: $requestId")
                sendNotification(
